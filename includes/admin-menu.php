...
/**
 * Retrieve remote dashboard content for display on the overview tab.
 *
 * @return array
 */
function rsm_get_dashboard_resource_panel() {
    $url      = 'https://chatgpt.com/backend-api/estuary/content?id=file_00000000d170720aafd1e161f3c70220&ts=490355&p=fs&cid=1&sig=e35564d7164be355dd77a185a1b269601711f6b87962d9e3afa53098d8750eaf&v=0';
    $defaults = array(
        'content' => '',
        'message' => __( 'Resource is currently unavailable.', 'race-series-manager' ),
        'url'     => $url,
    );
    ...
    if ( is_wp_error( $response ) ) {
        $defaults['message'] = $response->get_error_message();
        return $defaults;
    }
    ...
    $defaults['content'] = wp_kses( $body, $allowed_tags );
...
function rsm_render_admin_home() {
    ...
    $resource     = rsm_get_dashboard_resource_panel();
        ...
        <div class="rsm-dashboard-panels">
            <section id="rsm-tab-overview" class="rsm-dashboard-panel is-active" role="tabpanel">
                <div class="rsm-dashboard-grid">
                    ...
                    <div class="rsm-card">
                        <div class="rsm-card-header">
                            <span class="rsm-card-kicker"><?php esc_html_e( 'Resources', 'race-series-manager' ); ?></span>
                            <h2><?php esc_html_e( 'Latest update', 'race-series-manager' ); ?></h2>
                        </div>
                        <?php if ( ! empty( $resource['content'] ) ) : ?>
                            <div class="rsm-remote-panel">
                                <?php echo wp_kses_post( $resource['content'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                            </div>
                        <?php else : ?>
                            <p class="rsm-note"><?php echo esc_html( $resource['message'] ); ?></p>
                        <?php endif; ?>
                        <div class="rsm-card-actions">
                            <a class="button" href="<?php echo esc_url( $resource['url'] ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Open resource', 'race-series-manager' ); ?></a>
                        </div>
                    </div>
                </div>
            </section>
